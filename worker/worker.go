...
func (w *Worker) Search(request common.WorkerSearchRequest, response *common.WorkerSearchResponse) error {
	...
	data, err := w.readReduceOrReplicaFile(request.FileName)
	if err != nil {
		return err
	}
	...
	response.URLs = append([]string(nil), payload[keyword]...)
	return nil
}
...
func (w *Worker) shouldServeOnly() bool {
	w.mu.Lock()
	defer w.mu.Unlock()

	return w.keepAlive && w.searchReady
}
